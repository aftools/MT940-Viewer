...
class TagBalance extends Tag {
  ...
  _extractFields(match) {
    return {
      date: BankDate.parse(match[2], match[3], match[4]),
      currency: match[5],
      amount: BankAmount.parse(match[1], match[6])
    };
  }
}
...
class TagStatementLine extends Tag {
  ...
  _extractFields(match) {
    return {
      date: BankDate.parse(match[1], match[2], match[3]),
      entryDate: match[4] && BankDate.parse(match[1], match[5], match[6]) || '',
      fundsCode: match[8] || '',
      amount: BankAmount.parse(match[7], match[9]),
